...
def job_daily_pipeline():
    ...
    try:
        # 同日重複防止
        from db import get_recent_pipeline_runs
...
def _update_x_health(status: str, note: str = ""):
    ...
    try:
        from db import update_health as db_update_health
        db_update_health(
            "x_daemon",
            status,
            note=note,
...
def job_x_post_check():
    ...
    try:
        from x_post_local import post_next_from_db
...
def job_jobs_queue():
    ...
    try:
        from jobs import run_pending, get_stats
        stats = get_stats()
