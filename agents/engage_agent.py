...
def _slot_match() -> tuple[bool, bool]:
    ...
    try:
        from advisor import get_advice
        from slot_utils import is_now_in_slots
        from datetime import datetime, timezone, timedelta
...
def _generate_comment(tweet_text: str, mode: str) -> str:
    ...
    from llm_wrapper import call_llm
    if mode == "quote":
        prompt = f"""以下のツイートを引用RTする際の、自分のコメント(140字以内)を1つ生成してください。
...
- 共感 + 自分の観察や追加の気づきを添える
- 上から目線・教える口調はNG
- 「私は◯ヶ月で◯円稼いだ」など架空の実績は絶対禁止
- ハッシュタグなし
- 説明や前置き一切不要、コメント本文のみ出力
"""
    else:
        prompt = f"""以下のツイートに自然なリプライ(80字以内)を1つ生成してください。

【元ツイート】
{tweet_text[:300]}

【ルール】
- 共感や同意を示しつつ、自分の小さな観察を1つ添える
- 宣伝・URLは絶対NG
- 上から目線NG
- 説明や前置き一切不要、リプライ本文のみ出力
"""
    try:
        return call_llm(prompt, task_type="article_generation", temperature=0.8, max_tokens=200).strip()
...
def _record(action_type: str, url: str, text: str) -> None:
    try:
        from db import record_growth_action
