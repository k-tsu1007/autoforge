...
【書き方の原則】
- 元ツイートを読んで自分が感じた「引っかかり」「補足したいこと」「別の見方」を素直に書く
- 「自分の場合こう思う」「こういう視点もある」という自然な反応でいい
- 知識をひけらかすな。断定しない。「〜と思う」「〜な気がする」くらいの温度感
- 120字以内、ハッシュタグなし、URL禁止
- 「◯ヶ月で◯円稼いだ」など数値実績禁止
- コメントのみ出力（説明不要）

【こういう返し方がいい（雰囲気の参考）】
- 元ツイートの一部に「たしかに。ただ〜の場合は逆になりそう」
- 「これ、〜のケースだとどうなるんだろう。気になった」
- 「〜って盲点だった。自分もやってなかったな」
- 「逆に〜するとうまくいくパターンもある気がしてる」

【こういう返し方はだめ】
- 「この視点は非常に興味深いですね」（堅い）
- 「同感です！まさにそう思います！」（空っぽ）
- 「〇〇しましょう」「〇〇が大切です」（説教）
- テンプレっぽい「〇〇という観点も気になる。特に△△は〜」（作り物感）
"""
    else:
        prompt = f"""{persona}
...
【書き方の原則】
- 「読んで思ったこと」を短く書く。共感でも疑問でも気づきでもいい
- 返しとして自然に成立すればいい。「うまいことを言おう」としない
- 80字以内、ハッシュタグなし、URL禁止
- 「◯ヶ月で◯円稼いだ」など数値実績禁止
- リプライのみ出力（説明不要）

【こういう返し方がいい（雰囲気の参考）】
- 「これ、〜のときに感じてた。まだ答え出てないけど」
- 「〜って部分、自分もそうだったな。今は少し変わったけど」
- 「〜だとしたら、〜の場合はどうなるんだろう」
- 「そういう見方もあるんだ。ちょっと考えさせられた」

【こういう返し方はだめ】
- 「おっしゃる通りです！参考になります！」（空っぽ）
- 「〇〇という観点も重要ではないでしょうか」（堅い・上から）
- 何かを教えようとするリプライ（専門家ぶる）
"""
    try:
