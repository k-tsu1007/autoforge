...
【具体的なトーン】
- 元ツイートを読んで「何かが動いた」感を起点にする（発見・納得・興味・軽い驚き）
- 自分語りや苦労話ではなく、相手のツイートへの反応として書く
- 重くなりすぎない。軽さ・好奇心・前向きさがある温度感
- 120字以内、ハッシュタグなし、URL禁止、数値実績禁止
- コメントのみ出力

【良い例（雰囲気）】
- 「これ面白い視点。こっちも試したくなった」（発見・興味）
- 「なるほど、そういう整理の仕方があるか。たしかに」（納得）
- 「こういう切り口、好きだな。あともう一歩どうするかが気になる」（興味の続き）

【悪い例】
- 「非常に参考になります！」（空っぽ）
- 「ただ〇〇の場合は違うと思います」（否定）
- 「〇〇しましょう」（説教）
- 「自分もずっとそうで苦労してた」（暗い自分語り）
"""
    else:
        prompt = f"""{persona}
...
【具体的なトーン】
- 元ツイートを読んで何かが動いた部分（発見・納得・軽い疑問）を起点にする
- 相手に向けて書く。自分の内側に閉じない
- 重くならない。明るくしすぎなくていいが、暗い自分語りにしない
- 80字以内、ハッシュタグなし、URL禁止、数値実績禁止
- リプライのみ出力

【良い例（雰囲気）】
- 「そういう整理の仕方、なかったな。ちょっとやってみたくなった」（発見）
- 「なんか続きが気になる。うまくいったら教えてほしい」（興味・つながり）
- 「この視点、SNS以外にも使えそう。けっこう普遍的かも」（広がり）

【悪い例】
- 「おっしゃる通りです！参考になります！」（空っぽ）
- 「ただ〇〇は違うと思いますよ」（反論）
- 「〇〇が大切ですよね」（上から）
- 「自分もずっとそこで詰まってた」（暗い独り言）
"""
    try:
