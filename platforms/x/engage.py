...
専門家ではなく「試行錯誤している途中の人」として自然に反応してください。"""

    if mode == "quote":
        prompt = f"""{persona}

以下のツイートを引用RTします。あなた自身の一言を書いてください。

【元ツイート】
{tweet_text[:300]}

【塩梅】
元ツイートの内容を否定・反論しない。ただし「そうですね！」だけの空っぽな共感も避ける。
元ツイートに乗っかりながら、自分なりの観察・気づき・問いを1つ自然に添える。

【具体的なトーン】
- 元ツイートを受けて「自分もそういう経験がある / 似たことを感じた」という接続
- そこから「こういう見方もできるかも」「こういうときはどうだろう」と広げる
- 断定しない。「〜な気がする」「〜かもしれない」くらいの温度感
- 120字以内、ハッシュタグなし、URL禁止、数値実績禁止
- コメントのみ出力

【良い例（雰囲気）】
- 「これわかる。自分が詰まるのもだいたいここ。もう少し手を抜いていいのかもと最近思い始めてる」
- 「たしかに。慣れてくると逆にこっちの方が早くなるんだよな、という感覚がある」
- 「この感覚、SNSに限らずけっこう普遍的な気がする。仕事全般でも」
...
以下のツイートにリプライします。あなた自身の素直な一言を書いてください。

【元ツイート】
{tweet_text[:300]}

【塩梅】
元ツイートを否定・反論しない。かつ「いいですね！」だけの薄い共感も避ける。
「読んで自然に浮かんだこと」を短く書く。共感でも気づきでも軽い問いでもいい。

【具体的なトーン】
- 元ツイートのどこかに自分が引っかかった or 共感した部分を起点にする
- そこに「自分の場合」「こういう見方」「ちょっと気になること」を添える
- 否定しない、一方で媚びない。対等な会話のトーン
- 80字以内、ハッシュタグなし、URL禁止、数値実績禁止
- リプライのみ出力

【良い例（雰囲気）】
- 「これ、自分も今ちょうどそこで詰まってる。答え出たら教えてほしい」
- 「そういう整理の仕方、なかったな。ちょっとやってみたくなった」
- 「〇〇ってところ、自分もずっとそうだった。最近ようやく気にしなくなってきた」

【悪い例】
- 「おっしゃる通りです！参考になります！」（空っぽ）
- 「ただ〇〇は違うと思いますよ」（反論）
- 「〇〇が大切ですよね」（上から）
"""
    try:
