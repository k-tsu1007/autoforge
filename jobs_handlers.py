...
def _post_x(payload: dict) -> dict:
    ...
    from x_post_local import post_to_x
...
def _take_snapshot(payload: dict) -> dict:
    """メトリクススナップショット取得。"""
    from db import take_metrics_snapshot, get_strategy
...
def _backup_data(payload: dict) -> dict:
    """データをバックアップする。"""
    from maintenance import backup_data
    backup_data()
    return {"ok": True}
...
def _check_x_cookie(payload: dict) -> dict:
    """X Cookie期限チェック。"""
    from maintenance import check_x_cookie_expiry
