...
## タイトル形式の多様性チェック（必須）
記事タイトルを以下の形式に分類し、偏りを検出すること:
- How-to型: 「〇〇する方法」「〇〇のやり方」
- リスト型: 「△つの〇〇」「〇〇のN個の特徴」
- Why型: 「なぜ〇〇か」「〇〇がうまくいかない理由」
- 比較型: 「〇〇と〇〇の違い」「〇〇 vs 〇〇」
- 解説型: 「〇〇とは何か」「〇〇の仕組み」
- 誤解解消型: 「〇〇のよくある誤解」「〇〇する前に知ること」
- 提案・ツール型: 「テンプレート」「チェックリスト」「始め方」

特定の形式が3本以上連続している、または全体の50%超を占めている場合は:
program_updates に「〇〇型を避け、〇〇型・〇〇型を積極的に使うこと」という具体的な指示を追加すること。
データが少なくても（PV/スキが0でも）形式の偏りは検出・修正できる。
...
def evolve():
    ...
    try:
        from core.learning.forget import trim_program_md
